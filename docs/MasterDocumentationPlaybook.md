...
version: 1.1.0
subsystem: ansible
tags: [playbook, documentation, standards, governance, ci-enforcement]
doc_type: guide
---
...
Example Recommended top‑level layout:
...
docs/
  ...
  MasterDocumentationPlaybook.md                    # Documentation standards & metadata schema
  ...
  reference/
    crates/                       # Thin index pages that link to crate READMEs ONLY IF NEEDED
...
editor: Claude Code (Claude Opus 4.5)  # AI model name, or "human" if manual edit
user: Coldaine                          # Human who directed/reviewed
status: draft | ready | complete | deprecated
version: 1.0.0                          # Semver, maintain it
subsystem: monorepo | ansible | llm-chat-parser | claude-hooks-system | dead-mans-switch | ops
...
**Optional fields:**
```yaml
doc_type: reference | plan | guide | architecture | adr | research | index | playbook | troubleshooting
scope: monorepo | package:<name> | ops:<machine>
...
3) Ensure each domain has an overview document (typically `<code>-overview.md`) that links to all other domain documentation.
...
version: 0.1.0
subsystem: monorepo
tags: [research, pr-report, clipboard, ephemeral]
doc_type: research
---
...
- 1.1.0 (2026‑01‑20)
  - **Breaking**: Unified frontmatter standard across all repositories (§3)
  - Required fields: `last_edited`, `editor`, `user`, `status`, `version`, `subsystem`, `tags`
  - Optional fields: `doc_type`, `scope`, `supersedes`
  - Dropped: `owners` (not meaningful), `last_reviewed` (merged into `last_edited`)
  - Updated all examples (§11.1-11.3) to new format
- 1.0.1 (2025‑11‑06)

...
from agents.errors import AgentsToolCreateError
from agents.functions.schema_generator import generate_schema
...
def _get_module_source(module: ModuleType) -> str:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        if module_name.startswith("agents.functions.function_sets."):
            file_name = module_name.split(".")[-1] + ".py"
            bundled_path = os.path.join(
                sys._MEIPASS, "agents", "functions", "function_sets", file_name
            )

...
class SummarizerSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="agents_summarizer_", extra="ignore")
...
# read pg_uri from ~/.recall_agents/pg_uri or set to none, this is to support Mirix Desktop
default_pg_uri = None
...
if "--use-file-pg-uri" in sys.argv:
    try:
        with open(Path.home() / ".mirix/pg_uri", "r") as f:
            ...
            print("Read pg_uri from ~/.recall_agents/pg_uri")
...
class Settings(BaseSettings):
    ...
    agents_dir: Optional[Path] = Field(Path.home() / ".recall_agents", env="AGENTS_DIR")
    # Directory where uploaded/processed images are stored
    # Can be overridden with AGENTS_IMAGES_DIR environment variable
    images_dir: Optional[Path] = Field(
        Path.home() / ".recall_agents" / "images", env="AGENTS_IMAGES_DIR"
    )
    ...
    pg_uri: Optional[str] = Field(
        default_pg_uri, env="AGENTS_PG_URI"
    ...
    @property
    def agents_pg_uri(self) -> str:
        # Prefer explicit env var for full URI (standard pattern)
...
class TestSettings(Settings):
    model_config = SettingsConfigDict(env_prefix="agents_test_", extra="ignore")

    agents_dir: Optional[Path] = Field(Path.home() / ".recall_agents/test", env="AGENTS_TEST_DIR")
    images_dir: Optional[Path] = Field(

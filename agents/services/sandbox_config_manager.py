...
from agents.constants import AGENTS_DIR_TOOL_SANDBOX
from agents.log import get_logger
from agents.orm.errors import NoResultFound
from agents.orm.sandbox_config import SandboxConfig as SandboxConfigModel
from agents.orm.sandbox_config import SandboxEnvironmentVariable as SandboxEnvVarModel
from agents.schemas.environment_variables import (
...
from agents.schemas.sandbox_config import (
    LocalSandboxConfig,
    SandboxConfigCreate,
    SandboxConfigUpdate,
    SandboxType,
)
from agents.schemas.sandbox_config import SandboxConfig as PydanticSandboxConfig
from agents.schemas.user import User as PydanticUser
from agents.utils import enforce_types, printd
...
class SandboxConfigManager:
    ...
    @enforce_types
    def get_or_create_default_sandbox_config(
        self, sandbox_type: SandboxType, actor: PydanticUser
    ) -> PydanticSandboxConfig:
        sandbox_config = self.get_sandbox_config_by_type(sandbox_type, actor=actor)
        if not sandbox_config:
            logger.debug(
                f"Creating new sandbox config of type {sandbox_type}, none found for organization {actor.organization_id}."
            )
            ...
            if sandbox_type == SandboxType.E2B:
                ...
                default_local_sandbox_path = AGENTS_DIR_TOOL_SANDBOX
                default_config = LocalSandboxConfig(
                    sandbox_dir=default_local_sandbox_path
                ).model_dump(exclude_none=True)

            sandbox_config = self.create_or_update_sandbox_config(
                SandboxConfigCreate(config=default_config), actor=actor
            )
        return sandbox_config

    @enforce_types
    def create_or_update_sandbox_config(
        ...
        if db_sandbox:
            ...
            update_data = sandbox_config.model_dump(
                exclude_unset=True, exclude_none=True
            )
            update_data = {
                key: value
                for key, value in update_data.items()
                if getattr(db_sandbox, key) != value
            }

            # If there are changes, update the sandbox configuration
            if update_data:
                db_sandbox = self.update_sandbox_config(
                    db_sandbox.id, SandboxConfigUpdate(**update_data), actor
                )
            else:
                printd(
            ...
            with self.session_maker() as session:
                db_sandbox = SandboxConfigModel(
                    **sandbox_config.model_dump(exclude_none=True)
                )
    ...
    def update_sandbox_config(
        ...
        sandbox_update: SandboxConfigUpdate,
        actor: PydanticUser,
    ) -> PydanticSandboxConfig:
        """Update an existing sandbox configuration."""
        with self.session_maker() as session:
            sandbox = SandboxConfigModel.read(
                db_session=session, identifier=sandbox_config_id, actor=actor
            )
            ...
            update_data = sandbox_update.model_dump(
                exclude_unset=True, exclude_none=True
            )
            update_data = {
                key: value
    ...
    def list_sandbox_configs(
        ...
        with self.session_maker() as session:
            sandboxes = SandboxConfigModel.list(
                db_session=session, cursor=cursor, limit=limit, **kwargs
            )
            return [sandbox.to_pydantic() for sandbox in sandboxes]

    @enforce_types
    def get_sandbox_config_by_id(
        self, sandbox_config_id: str, actor: Optional[PydanticUser] = None
    ...
    @enforce_types
    def get_sandbox_config_by_type(
    ...
    def create_sandbox_env_var(
        ...
        env_var = PydanticEnvVar(
            **env_var_create.model_dump(),
            sandbox_config_id=sandbox_config_id,
            organization_id=actor.organization_id,
        )

        db_env_var = self.get_sandbox_env_var_by_key_and_sandbox_config_id(
            env_var.key, env_var.sandbox_config_id, actor=actor
        )
        if db_env_var:
            ...
            update_data = {
                key: value
                for key, value in update_data.items()
                if getattr(db_env_var, key) != value
            }
            # If there are changes, update the environment variable
            if update_data:
                db_env_var = self.update_sandbox_env_var(
                    db_env_var.id,
                    SandboxEnvironmentVariableUpdate(**update_data),
                    actor,
                )
            else:
                printd(
    ...
    def update_sandbox_env_var(
        ...
        with self.session_maker() as session:
            env_var = SandboxEnvVarModel.read(
                db_session=session, identifier=env_var_id, actor=actor
            )
            update_data = env_var_update.model_dump(
                exclude_unset=True, exclude_none=True
            )
            update_data = {
                key: value
    ...
    def delete_sandbox_env_var(
        self, env_var_id: str, actor: PydanticUser
    ) -> PydanticEnvVar:

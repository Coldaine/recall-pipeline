...
from agents import EmbeddingConfig, LLMConfig, create_client
from agents.agent.agent_configs import AGENT_CONFIGS
from agents.agent.agent_states import AgentStates
...
from agents.agent.message_queue import MessageQueue
from agents.agent.temporary_message_accumulator import TemporaryMessageAccumulator
from agents.agent.upload_manager import UploadManager
from agents.prompts import gpt_system
from agents.schemas.agent import AgentType
from agents.schemas.memory import ChatMemory
from agents.schemas.agents_message import MessageType
...
class AgentWrapper:
    ...
    def _restore_database_before_init(self, folder_path: str):
        ...
        try:
            ...
            if backup_type == "postgresql":
                if not settings.agents_pg_uri_no_default:
                    raise ValueError(
    ...
    def _check_and_connect_gmail(self):
        """
        Check if Gmail credentials exist in ~/.recall_agents and connect to Gmail MCP if they do.
        ...
        try:
            ...
            if not credentials_file.exists():
                self.logger.debug("No Gmail credentials found in ~/.recall_agents")
                return False
    ...
    def set_persona(self, persona_name):
        ...
        from agents.prompts import gpt_persona
...
Please perform this analysis and create new memories as appropriate. Provide a detailed report of patterns found and new memories created."""
            ...
            if settings.agents_pg_uri_no_default:
                ...
                try:
                    ...
                    agent_config = {
                        ...
                        "backup_type": "postgresql",
                        "backup_timestamp": utcnow().isoformat(),
                        "connection_info": {
                            "db_host": db_host,
                            "db_port": db_port,
                            "db_user": db_user,
    ...
    def create_user(self, name: str, set_as_active: bool = False) -> dict:
        ...
        try:
            ...
            from agents.schemas.user import User as PydanticUser
            from agents.services.organization_manager import OrganizationManager

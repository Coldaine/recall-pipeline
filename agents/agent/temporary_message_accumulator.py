...
from agents.agent.app_utils import encode_image
from agents.constants import CHAINING_FOR_MEMORY_UPDATE
from agents.voice_utils import convert_base64_to_audio_segment, process_voice_files
...
class TemporaryMessageAccumulator:
    ...
    def absorb_content_into_memory(
        ...
        if voice_content:
            current_timestamp = utcnow().strftime("%Y%m%d_%H%M%S_%f")[
                :-3
            ]

...
from agents.functions.functions import (
    derive_openai_json_schema,
    get_json_schema_from_module,
)
from agents.functions.helpers import generate_langchain_tool_wrapper
from agents.functions.schema_generator import generate_schema_from_args_schema_v2
from agents.orm.enums import ToolType
from agents.schemas.agents_base import AgentsBase


class BaseTool(AgentsBase):
    __id_prefix__ = "tool"
    ...
    def populate_missing_fields(self):
        ...
        if self.tool_type == ToolType.CUSTOM:
            ...
            self.json_schema = get_json_schema_from_module(
                module_name=AGENTS_CORE_TOOL_MODULE_NAME, function_name=self.name
            )
        elif self.tool_type in {ToolType.MIRIX_MEMORY_CORE}:
            self.json_schema = get_json_schema_from_module(
                module_name=AGENTS_MEMORY_TOOL_MODULE_NAME, function_name=self.name
            )
        elif self.tool_type in {ToolType.MIRIX_EXTRA}:
            self.json_schema = get_json_schema_from_module(
                module_name=AGENTS_EXTRA_TOOL_MODULE_NAME, function_name=self.name
            )
...
class ToolRunFromSource(AgentsBase):
    source_code: str = Field(..., description="The source code of the function.")

...
from agents.schemas.enums import MessageStreamStatus
from agents.schemas.agents_message import AgentsMessage, AgentsMessageUnion
...
class AgentsResponse(BaseModel):
    ...
    The type of the returned messages can be either `Message` or `AgentsMessage`, depending on what was specified in the request.

    Attributes:
        messages (List[Union[Message, AgentsMessage]]): The messages returned by the agent.
        usage (AgentsUsageStatistics): The usage statistics
    """

    messages: List[AgentsMessageUnion] = Field(
        ...,
        description="The messages returned by the agent.",
        json_schema_extra={
            "items": {
                "$ref": "#/components/schemas/AgentsMessageUnion",
            }
        },
    )
    usage: AgentsUsageStatistics = Field(
        ...,
        ...
        return json_dumps(
            {
                ...
                # Assume `Message` and `AgentsMessage` have a `dict()` method
                "usage": self.usage.model_dump(),  # Assume `AgentsUsageStatistics` has a `dict()` method
            },
            indent=4,
        )
...
# The streaming response is either [DONE], [DONE_STEP], [DONE], an error, or a AgentsMessage
AgentsStreamingResponse = Union[AgentsMessage, MessageStreamStatus, AgentsUsageStatistics]

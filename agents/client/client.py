...
from agents.functions.functions import parse_source_code
from agents.interface import QueuingInterface
from agents.orm.errors import NoResultFound
from agents.schemas.agent import AgentState, AgentType, CreateAgent, UpdateAgent
from agents.schemas.block import Block, BlockUpdate, CreateBlock, Human, Persona
...
from agents.schemas.file import FileMetadata
...
from agents.schemas.message import Message, MessageCreate, MessageUpdate
...
from agents.schemas.agents_response import AgentsResponse
from agents.schemas.openai.chat_completions import ToolCall
...
from agents.schemas.tool import Tool, ToolCreate, ToolUpdate
from agents.schemas.tool_rule import BaseToolRule
...
class AbstractClient(object):
    ...
    def user_message(self, agent_id: str, message: str) -> AgentsResponse:
        raise NotImplementedError
...
class LocalClient(AbstractClient):
    ...
    def __init__(
        ...
        from agents.server.server import SyncServer

        # set logging levels
        agents.utils.DEBUG = debug
    ...
    def send_messages(
        ...
        Returns:
            response (AgentsResponse): Response from the agent
        """
        self.interface.clear()
        ...
        # format messages
        return AgentsResponse(messages=messages, usage=usage)
    ...
    def send_message(
        ...
        agents_messages = []
        for m in messages:
            agents_messages += m.to_agents_message()

        return AgentsResponse(messages=agents_messages, usage=usage)
    ...
    def run_command(self, agent_id: str, command: str) -> AgentsResponse:
        """
        ...
        Returns:
            AgentsResponse: The response from the agent
        ...
        return AgentsResponse(messages=self.interface.to_list(), usage=usage)

...
from agents.schemas.openai.chat_completion_request import Tool, ToolCall
...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from agents.openai_backcompat.openai_object import OpenAIObject
...
def get_local_time_military():
    ...
    current_time_utc = utcnow()

    # Convert to San Francisco's time zone (PST/PDT)
    sf_time_zone = ZoneInfo("America/Los_Angeles")
...
def get_local_time_timezone(timezone="America/Los_Angeles"):
    ...
    # Convert to San Francisco's time zone (PST/PDT)
    sf_time_zone = ZoneInfo(timezone)
    local_time = current_time_utc.astimezone(sf_time_zone)
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(AGENTS_DIR, "agents")
...
def list_human_files():
    ...
    defaults_dir = os.path.join(agents.__path__[0], "humans", "examples")
    user_dir = os.path.join(AGENTS_DIR, "humans")

    agents_defaults = os.listdir(defaults_dir)
    agents_defaults = [
        os.path.join(defaults_dir, f) for f in agents_defaults if f.endswith(".txt")
    ]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return agents_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(agents.__path__[0], "personas", "examples")
    user_dir = os.path.join(AGENTS_DIR, "personas")
...
def get_friendly_error_msg(
    ...
    from agents.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
